export const PROMPT_ORDER_APPNAME = 0;
export const PROMPT_ORDER_REACTFRAMEWORKS = 10;
export const PROMPT_ORDER_EXPRESSFEATURES = 10;
export const PROMPT_ORDER_REACTFRAMEWORK = 11;
export const PROMPT_ORDER_REACTBUNDLER = 12;
export const PROMPT_ORDER_REACTFEATURES = 13;
export const PROMPT_ORDER_FEATURES = 20;

export const FEATURE_ORDER_GH = -1;
export const FEATURE_ORDER_GIT = 0;
export const FEATURE_ORDER_PACKAGEJSON = 1;
export const FEATURE_ORDER_UNORDERED = 1000000;
